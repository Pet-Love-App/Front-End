...
export function FavoritesTab({ onOpenPost }: Props) {
  ...
  const toggleFavorite = async (postId: number, wasFavorited: boolean) => {
    try {
      ...
      setList(prev => prev.filter(p => !(p.id === postId && res.action === 'unfavorited')).map(p => p.id === postId ? { ...p, is_favorited: res.action === 'favorited', favorites_count: res.favorites_count ?? p.favorites_count } : p));
          ...
          <XStack gap="$3" alignItems="center">
            ...
            >
              <XStack alignItems="center" gap="$1">
                <Text fontSize="$5" color={item.is_favorited ? '$yellow10' : '$gray10'}>{item.is_favorited ? '★' : '☆'}</Text>
